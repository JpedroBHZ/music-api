...
//@ControllerAdvice: Avisa ao Spring que esta classe vai interceptar erros globais da controller
@ControllerAdvice
public class ControllerExceptionHandler {
    ...
    //@ExceptionHandler: Intercepta a nossa exceção customizada de recurso não encontrado
    ...
    public ResponseEntity<StandardError> resourceNotFound(ResourceNotFoundException e, HttpServletRequest request) {
        ...
        // Monta o StandardError com a mensagem que lançamos lá na Service
        StandardError err = new StandardError(
                Instant.now(),
                status.value(),
        ...
        // Retorna o JSON amigável com status 404
        return ResponseEntity.status(status).body(err);
    }
}